...
{         static final boolean masterDebugging    = true,
  ...
  { 
    addPlayer(new ShavlikPlayer(this, showSensors), name, color);
  }
  ...
  { 
    addSmartPlayer(name, ShavlikPlayer.ShavlikPlayerColor, showSensors);
  }
  public void addSmartPlayer(String name)
  { 
    addSmartPlayer(name, ShavlikPlayer.ShavlikPlayerColor);
  }
  public void addAnonymousSmartPlayers(int number, Color color)
  { 
    for(int i = 0; i < number; i++)
    { // Don't assign a Score component to these.
      playingField.addPlayer(new ShavlikPlayer(this), "an anonymous Smart Player", color, null);
    }
  }
  public void addAnonymousSmartPlayers(int number)
  { 
    addAnonymousSmartPlayers(number, ShavlikPlayer.ShavlikPlayerColor);
  }
